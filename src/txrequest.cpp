...
struct ByPeerViewExtractor
{
    ...
    {
        return ByPeerView{ann.m_peer, ann.m_state == State::CANDIDATE_BEST, ann.m_inv};
    }
};
...
public:
    ...
    result_type operator()(const Announcement& ann) const
    {
        ...
        return ByTxHashView{ann.m_inv, ann.m_state, prio};
    }
};
...
private:
    ...
    void ChangeAndReselect(Iter<ByTxHash> it, State new_state)
    {
        ...
        if (it->IsSelected() && it != m_index.get<ByTxHash>().begin()) {
            ...
            if (it_prev->m_inv == it->m_inv && it_prev->m_state == State::CANDIDATE_READY) {
...
public:
    ...
    bool ReceivedResponse(NodeId peer, const CInv& txhash)
    {
        ...
        if (it == m_index.get<ByPeer>().end() || it->m_state == State::COMPLETED) return false;
        MakeCompleted(m_index.project<ByTxHash>(it));
        return true;
    }
